...
colnames(x) <- c('Max.self coverage', 'Min. Jaccard', 'Mbps')
...
p <- ggplot(x, aes(x=`Min. Jaccard`, y=Mbps, color=`Max.self coverage`)) + 
  geom_line() + 
  theme_bw() + 
  facet_grid(~`Max.self coverage`) +
  scale_x_continuous(breaks = round(seq(min(x$`Min. Jaccard`), max(x$`Min. Jaccard`), by = 0.01), 3)) +
