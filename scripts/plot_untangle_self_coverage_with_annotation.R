...
#xx[xx$self.coverage > 2,]$self.coverage <- 2
...
p <- ggplot(
  ...
  theme(
    ...
    plot.margin = unit(c(0,0.43,0,7.35), "cm"),
  ) +
  scale_x_continuous(limits = c(x_min, x_max), expand = c(0, 0)) +
  scale_y_continuous(limits = c(1, max(xx$self.coverage)), trans='log10') +
  scale_fill_manual(values = colors) +
  labs(x = "Position", y= "Self coverage", fill="Target") 
