...
x <- read.delim(
  '/home/guarracino/Downloads/Pangenomics/chromosome_communities/Review1/chrSEX_7-Dec-22_PHRs.bed',
  header = F
) %>% select(V1, V2, V3)
colnames(x) <- c('chrom', 'begin', 'end')

x$color <- 'PHRs'


x$chrom <- paste0(x$chrom, '#PHRs')

y <- read.delim(
  '/home/guarracino/git/chromosome_communities/data/chm13_hg002.PARs.bed',
  header = F
)
colnames(y) <- c('chrom', 'begin', 'end')

y$color <- str_sub(y$chrom, start = 1, end = 4)

z <- rbind(x, y)
z$group <- str_sub(z$chrom, start = 1, end = 4)
...
z$chrom <- factor(
  z$chrom,
  levels = rev(c(
    "chrX#PAR1", "chrX#XTR", "chrX#PAR2", "chrX#PHRs",
    "chrY#PAR1", "chrY#XTR1", "chrY#XTR2", "chrY#PAR2", "chrY#PHRs"
  ))
)
