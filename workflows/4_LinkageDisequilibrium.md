...
paste -d'\n' chrNames.txt acrocentrics.txt | while read f1 && read f2 ; do plink --vcf chrACRO+refs.vcf.gz --allow-extra-chr  --chr-set -5 --chr $f1 --r2  --extract 'range' data/annotation/chm13.p_arms.approximate.acros.bed --ld-window-kb 10  --ld-window-r2 0 --out ld/$f2.pArm; done
```
q arm
```
paste -d'\n' chrNames.txt acrocentrics.txt | while read f1 && read f2 ; do plink --vcf chrACRO+refs.vcf.gz --allow-extra-chr  --chr-set -5 --chr $f1 --r2  --extract 'range' data/annotation/chm13.q_arms.approximate.acros.bed --ld-window-kb 10  --ld-window-r2 0 --out ld/$f2.qArm; done
```

putative recombinant
```
paste -d'\n' chrNames.txt acrocentrics.txt | while read f1 && read f2 ; do plink --vcf chrACRO+refs.vcf.gz --allow-extra-chr  --chr-set -5 --chr $f1 --r2  --extract 'range' chrARCO_25-Jul-22_PPRRs.bed --ld-window-kb 10  --ld-window-r2 0 --out ld/$f2.recombinant; done
```
